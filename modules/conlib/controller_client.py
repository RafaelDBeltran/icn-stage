...
#### WORKERS CONTROL METHODS
	...
	def worker_get(self, worker_path):
		...
		try:
			...
			logging.debug('[1]ERROR worker_path {}'.format( type(worker_path) ))

			
			worker_hostname = worker_path.split('/')[-1]
			logging.debug('[2]ERROR worker_hostname %s', worker_hostname)


			worker_username,_ = self.zk.get("%s/user" % (worker_path))
			logging.debug('[3]ERROR worker_username {}'.format(worker_username))


			worker_password,_ = self.zk.get("%s/password" % (worker_path))
			logging.debug('[4]ERROR worker_password {}'.format(worker_password))


			worker_pkey,_ = self.zk.get("%s/pkey" % (worker_path))
			logging.debug('[5]ERROR worker_pkey {}'.format(worker_pkey))


			worker_connection,_ = self.zk.get("%s/connection" % (worker_path))
			logging.debug('[6]ERROR worker_connection {}'.format(worker_connection))


			worker_status = self.worker_get_status(worker_path)
			logging.debug('[7]ERROR worker_status 7 {}'.format(worker_status))


			worker_active_time,_ = self.zk.get("%s/active_time" % (worker_path))
			logging.debug('[8]ERROR worker_active_time {}'.format(worker_active_time))


			worker_disconnection_time,_ = self.zk.get("%s/disconnection_time" % (worker_path))
			logging.debug('[9]ERROR worker_disconnection_time {}'.format(worker_disconnection_time))


			worker_connection_time,_ = self.zk.get("%s/connection_time" % (worker_path))
			logging.debug('[10]ERROR worker_connection_time {}'.format(worker_connection_time))


			worker_failures,_ = self.zk.get("%s/failures" % (worker_path))
			logging.debug('[11]ERROR worker_failures {}'.format(worker_failures))


			worker_actors = []
			for i in self.zk.get_children("%s/torun" % (worker_path)):
				logging.debug('[12]ERROR worker_path %s', worker_path)
				logging.debug('[13]ERROR CL ')
				logging.debug('[13]ERROR CL worker_path {}'.format(type(worker_path)))
				logging.debug('[13]ERROR CL E {}'.format(type(i)))
