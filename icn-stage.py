...
def get_address_servers(servers_list = ''):


	for i, j in enumerate(data['controllers']):
	
		servers_list += j['remote_hostname']+':'
		servers_list += j['ClientPort']
	
		if i == (len(data['controllers'])-1):
...
def experiment_skeleton(experiment_name, commands, controller_client, experiment_dir=None, experiment_file_name=None):
    ...
    experiment_name = '%s_%s' % (experiment_name, 		     
    datetime.datetime.now().strftime(TIME_FORMAT).replace(':','-').replace(',','-'))
...
def run_command(zookeeper_controller, command, options=None):
    ...
    if command == 'start':
    	...
    	os.system('python3 play_test_ensemble.py')
        ...
        try:
            ...
            cmd = ['python3 {}'.format('tcp_client.py'), '--host {}'.format(zookeeper_controller.get_ip_adapter()),
            '--port {}'.format(test_port)]
            experiment_skeleton('test_tcp', cmd, zookeeper_controller.controller_client,"experiments/test_tcp/", "test_tcp.tar.gz")
        ...
        try:
            ...
            try:
                ...
                cmd_iperf = 'iperf --server --window 1024 --port {} --interval {} --format m --time {} {} '.format(iperf_port, 
                iperf_interval, server_time_secs, transport)
                ...
                cmd = ['python3', 'iperf_client.py', '--host', zookeeper_controller.get_ip_adapter(), '--port', iperf_port,
                '--time', client_time_secs, transport]
                experiment_skeleton('iperf', cmd, zookeeper_controller.controller_client, "experiments/iperf/", "iperf.tar.gz")
        ...
        try:
            ...
            cmd = ['python3', 'poke.py','{}'.format(zookeeper_controller.get_ip_adapter())]
            experiment_skeleton('test_ndn', cmd, zookeeper_controller.controller_client, "experiments/test_ndn/", "test_ndn.tar.gz")
            call_ndn_exp()
        ...
        try:
            ...
            cmd = ['python3', 'traffic_client.py', '{}'.format(zookeeper_controller.get_ip_adapter())]
            experiment_skeleton('ndn_traffic_generator', cmd, zookeeper_controller.controller_client, "experiments/test_traffic/",
            "test_traffic.tar.gz")
            call_ndn_traffic()
...
def main():
    ...
    set_logging()
    print(get_address_servers())
    zookeeper_controller = ZookeeperController(ensemble=True, ip_list=get_address_servers())
